...
<html lang="en">
  <head>
    ...
    <title>${operation} Salesman for ${APP_TITLE}</title>
    <style>
        #state {
        display: block;
        font-weight: bold;
...
<body>
    <div class="container-fluid">
        ...
        <#if success?? >
            <div>
                <p>
                <div class="alert alert-success" role="alert">Successfully ${operation}ed Salesman: <b>${nickName}</b></div>
                    <br>
        ...
        <#else>
            <div class="new_post_form">
                <form action="/update/salesman" method="POST">
                    <#include "/common/errors.ftl">
                    <h2>Add Salesman to the system</h2>
                    <div class="row"><div class="left">NickName:</div> <div class="right">${nickName!""}></div></div>
                    <input type="hidden" name="nickName" value="${nickName!""}">
                    <#include "/common/name.ftl">
                    <div class="row"><div class="left">Commission:</div> <div class="right"> <input type="text" id="commission" placeholder="commission" name="commission" size="120" value="${commission!""}"></div></div>
                    ...
                    <input type="submit" class="search" value="${operation} Salesman">
                </form>
            </div>
        </#if>
